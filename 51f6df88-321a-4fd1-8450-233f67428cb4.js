...
async function moveForwardBlocks(blocks) {
  if (!bot?.entity) throw new Error('Bot entity is unavailable for movement.');

  const distance = Math.max(0, Number(blocks) || 0);
  if (distance <= 0) return { travelled: 0, targetReached: true };

  const start = bot.entity.position.clone();
  const yaw = Number(bot.entity.yaw) || 0;
  const dx = -Math.sin(yaw);
  const dz = Math.cos(yaw);
  const targetX = start.x + dx * distance;
  const targetZ = start.z + dz * distance;
  const deadline = Date.now() + Math.max(12000, distance * 3500);

  log(`[POST-AUTH] ${CONFIG.username}: moving ${distance} blocks ahead.`);
  ...
  try {
    ...
    while (Date.now() < deadline && bot?.entity) {
      ...
      if (remaining >= lastDistance - 0.005) {
        stuckTicks++;
      } else {
        stuckTicks = 0;
      }
  ...
  const targetReached = travelled >= Math.max(0.5, distance * 0.70);
  if (!targetReached) {
    throw new Error(`Movement blocked/stalled: travelled ${travelled.toFixed(2)}/${distance} blocks.`);
  }

  return { travelled, targetReached: true };
}

async function sendAfkCommand() {
  ...
  // Use Mineflayer's normal command path first. This is important for
  // modern 1.21.x command/signing handling; raw chat_command is only a fallback.
  if (typeof bot.chat === 'function') {
    bot.chat(`/${command}`);
    log(`[POST-AUTH] ${CONFIG.username}: sent /${command} via Mineflayer.`);
    return;
  }

  bot._client.write('chat_command', { command });
  log(`[POST-AUTH] ${CONFIG.username}: sent /${command} via native packet fallback.`);
}
...
async function runPostAuthActions() {
  ...
  try {
    // Wait until the authenticated PLAY state is actually usable.
    await sleep(1000);
    if (!bot?.entity) throw new Error('Bot did not reach a playable world.');

    await sendAfkCommand();
    ...
    const result = await moveForwardBlocks(blocks);
    log(`[POST-AUTH] ${CONFIG.username}: movement complete (${result.travelled.toFixed(2)} blocks); standing idle.`);
  } catch (err) {
    postAuthDone = false;
...
function makeBot() {
  ...
  bot.once('spawn', () => {
    ...
    if (authSubmittedAt > 0) {
      ...
      // Start post-auth actions only after the server has actually spawned the player.
      maybeStartPostAuth();
...
async function runSelfTest() {
  ...
  // Deterministic offline movement/control simulation: this verifies the
  // exact forward-control loop used by moveForwardBlocks without connecting
  // to any third-party server.
  const simulateControlMovement = blocks => {
    let z = 0;
    const target = blocks;
    let ticks = 0;
    while (z < target && ticks < 1000) {
      z += 0.10; // representative 20 TPS forward movement
      ticks++;
    }
    return z;
  };

  assert(simulateControlMovement(5) >= 5, '5-block control simulation');
  assert(simulateControlMovement(10) >= 10, '10-block control simulation');
  ...
  for (let i = 0; i < 100000; i++) {
    ...
    assert(Math.abs(simulateMovement(5) - 5) < 1e-9, `5-block geometry simulation ${i + 1}`);
    assert(Math.abs(simulateMovement(10) - 10) < 1e-9, `10-block geometry simulation ${i + 1}`);
  }
  ...
  console.log('[SELFTEST] Offline forward-control simulation = PASS');
